...
<div id="body">
	<div class="navbar navbar-default" style="background-color:#CC99FF; height:10px; margin-bottom: initial; font-size: 12px;">
			<div class="container" >
				...
				<ul class="nav navbar-nav navbar-right" style="margin-top: 14px;">
					<a href="http://localhost/CI/index.php/Login_Controllers/dangnhap" style="color: #FFFF33;"><span class="glyphicon glyphicon-log-in"></span> Ðang nhập/Ðang ký</a>
				</ul>
			</div>
	</div>
                        ...
                        <div class="btn-group dropright">
                          <button class="btn btn-secondary dropdown-toggle" type="button" id="dropdownMenuButton" data-toggle="dropdown" aria-haspopup="true" aria-expanded="false" style="background:#337AB7;color: white; font-size: 16px;margin-bottom: 10px"> Quản lý đơn vị</button>
                              <div class="dropdown-menu" aria-labelledby="dropdownMenuButton">
                                ...
                                <li><a class="dropdown-item" href="http://localhost/CI/index.php/Donvi_Controller/them_donvi">Thêm mmới đơn vị</a></li>
                              </div>
                        </div>		
					</ul>
				</div>
   ...
   <div class="col-md-10">
			<div class="row-1" >
				<h2><strong>Sửa đơn vị</strong></h2>
                    <form action="<?php echo base_url(); ?>Donvi_Controller/sua_donvi/<?php echo $info['id']; ?>" method="POST">
    					<select name="id_parent" style="width: 80%; display: inline-block; border: none; background: #DDDDDD; padding: 15px; margin: 5px 0 22px 0;">
                            ...
                            <option <?php echo $info['id_parent']==$local['id']?'selected':''?> value="<?php echo $local['id'] ?>">|--<?php echo $local['tendv'] ?></option>  
                            <?php
							}
				            ?>
                            </select>
                            <h4 style="color: black;">Tên đơn vị</h4><input type="text" name="tendonvi" value="<?php echo $info['tendv'] ?>" style="width: 80%;" required/>
                            <h4 style="color: black;">Địa chỉ</h4><input type="text" name="diachi" value="<?php echo $info['diachi'] ?>" style="width: 80%;" required/>
                            <h4 style="color: black;">Email</h4><input type="text" name="email" value="<?php echo $info['email'] ?>" style="width: 80%;" required/>
                            <h4 style="color: black;">Số điện thoại</h4><?php echo form_error('sdt'); ?><input type="text" name="sdt" value="<?php echo $info['sdt'] ?>" style="width: 80%;" required/>
                            <input type="submit" class="btn btn-primary" value="Sửa đơn vị" style="width: 30%; height: 45px; margin-left: 509px; font-size: 18px;"/></a>
                    </form>
            </div>
    </div>
</div>
