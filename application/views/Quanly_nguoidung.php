	...
	<div class="navbar navbar-default" style="background-color:#CC99FF; height:10px; margin-bottom: initial; font-size: 12px;">
			<div class="container" >
				...
				<ul class="nav navbar-nav navbar-right" style="margin-top: 14px;">
					<a href="http://localhost/CI/index.php/Login_Controllers/login" style="color: #FFFF33;"> ,Đăng xuất <span class="glyphicon glyphicon-log-in"></span></a>
                </ul>
			</div>
	</div>
	...
	<div class="row" style="margin-bottom: -240px;">
				...
				<div class="sidebar-left">
					...
					<ul class="sidebar-menu" style="margin-left: -40px;">
                        ...
                        <div class="btn-group dropright">
                          ...
                          <div class="dropdown-menu" aria-labelledby="dropdownMenuButton">
                                <li><a class="dropdown-item" href="">Danh sách biểu mẫu</a></li>
                                <li><a class="dropdown-item" href="">Lịch sử tạo biểu mẫu</a></li>
				          </div>
                        </div>
                        <div class="btn-group dropright">
			...
			<div class="row-1" >
				<h2><strong>Quản lý người dùng</strong></h2>
					<p>Danh sách các người dùng đã đăng ký</p>            
					<table class="table table-striped" style="font-size: 16px; text-align: center;">
						<thead>
						<tr>
							<th style="text-align: center;"><h4><strong>STT</strong></h4></th>
							<th style="text-align: center;"><h4><strong>Tên hiển thị</strong></h4></th>
							<th style="text-align: center;"><h4><strong>Email</strong></h4></th>
							<th style="text-align: center;"><h4><strong>Địa chỉ</strong></h4></th>
							<th style="text-align: center;"><h4><strong>Đơn vị</strong></h4></th>
							<th style="text-align: center;"><h4><strong>Chọn</strong></h4></th>
						</tr>
						</thead>
						<?php
						...
						?>
						<tbody>
						<tr id="row-<?php echo $local['id']; ?>">
							<td><?php echo $local['id'] ?></td>
							<td><?php echo $local['display_name'] ?></td>
							<td><?php echo $local['email'] ?></td>
							<td><?php echo $local['address'] ?></td>
							<td><?php echo $local['donvi'] ?></td>
							<td><a href="<?php echo base_url('Quantri_Controllers/sua_nguoidung/'.$local['id']);?>"><input type="submit" class="btn btn-primary" name="sua" value="Sửa" style="height: 32px;" />
							<a class="delnguoidung" data-id="<?php echo $local['id']; ?>" href="javascript:void(0)"><input type="submit" class="btn btn-primary" name="xoa" value="Xóa" style="height: 32px;"/></a></td>
						</tr>
						<?php
							}
						?>
				...
				<div class="row-2" style="width: 145px; float: right; ">
					<p><a href="<?php echo base_url('Quantri_Controllers/them_nguoidung') ?>"><input type="submit" class="btn btn-primary" name="them" value="Thêm Người Dùng" /></p>		
				</div>
				<div class="row">
				</div>
			</div>
...
<script type="text/javascript">
    $(document).ready(function() {
        $('.delnguoidung').on('click', function() {
          var _id = $(this).data('id');
             alert(_id);
            if(confirm('Bạn muốn xóa người dùng này ?')){
                $.post(
                '<?php echo base_url('Quantri_Controllers/xoa_nguoidung/');?>',
                {
                    id: _id
                }, function(response) {
                    if(response){
