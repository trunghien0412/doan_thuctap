...
<div id="body">
   ...
   <div class="col-md-10">
			<div class="row-1" >
				<h2><strong>Quản lý địa giới</strong></h2>
					<p>Danh sách các địa giới hành chính</p>            
					<table class="table table-striped" style="font-size: 16px; text-align: center;">
						...
						<tr>
							...
							<th style="text-align: center;"><h4><strong>Tên địa giới</strong></h4></th>
						...
						<tr id="row-<?php echo $local['id']; ?>">
							...
							<td><?php echo $local['ten_diagioi'] ?></td>
							<td><a href="<?php echo base_url('Diagioi_Controller/sua_diagioi/'.$local['id']);?>"><input type="submit" class="btn btn-primary" name="sua" value="Sửa" style="height: 32px;" /></a>
							<a class="deldiagioi" data-id="<?php echo $local['id']; ?>" href="javascript:void(0)"><input type="submit" class="btn btn-primary" name="xoa" value="Xóa" style="height: 32px;"/></a></td>
						</tr>
						<?php
							}
						?>
				...
				<div class="row-2" style="width: 145px; float: right; ">
					<p><a href="<?php echo base_url('Diagioi_Controller/them_diagioi') ?>"><input type="submit" class="btn btn-primary" name="them" value="Thêm mới địa giới" /></p>		
				</div>
				<div class="row">
				</div>
			</div>
...
<script type="text/javascript">
    $(document).ready(function() {
        $('.deldiagioi').on('click', function() {
          var _id = $(this).data('id');
             alert(_id);
            if(confirm('Bạn chắc chắn muốn xóa?')){
                $.post(
                '<?php echo base_url('Diagioi_Controller/xoa_diagioi/');?>',
                {
                    id: _id
                }, function(response) {
                    if(response){
