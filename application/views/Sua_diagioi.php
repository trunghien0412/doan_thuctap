...
<div id="body">
	...
	<div class="row" style="margin-bottom: -240px;">
            ...
            <div class="col-md-10">
			<div class="row-1" >
				<h2><strong>Sửa địa giới</strong></h2>
                    <form action="<?php echo base_url(); ?>Diagioi_Controller/sua_diagioi/<?php echo $info['id']; ?>" method="POST">
    					<select name="id_parent" style="width: 80%; display: inline-block; border: none; background: #DDDDDD; padding: 15px; margin: 5px 0 22px 0;">
                            ...
                            foreach($arr_Users2 as $local){ ?>
                                <option <?php echo $info['parent_id']==$local['id']?'selected':''?> value="<?php echo $local['id'] ?>">|--<?php echo $local['ten_diagioi'] ?></option>
                           <?php
                            }
				            ?>
                            <input type="text" name="ten_diagioi" style="width: 80%;" value="<?php echo $info['ten_diagioi'] ?>" required/>
                            <input type="submit" class="btn btn-primary" value="Sửa địa giới" style="width: 30%; height: 45px; margin-left: 509px; font-size: 18px;"/></a>
    					</select>
                    </form>
            </div>
			</div>
