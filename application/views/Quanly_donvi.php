...
<div id="body">
	<div class="navbar navbar-default" style="background-color:#CC99FF; height:10px; margin-bottom: initial; font-size: 12px;">
			<div class="container" >
				...
				<ul class="nav navbar-nav navbar-right" style="margin-top: 14px;">
					<a href="http://localhost/CI/index.php/Login_Controllers/dangnhap" style="color: #FFFF33;"><span class="glyphicon glyphicon-log-in"></span> Đăng nhập/Đăng ký</a>
				</ul>
			</div>
	</div>
   ...
   <div class="col-md-10">
			<div class="row-1" >
				<h2><strong>Quản lý đơn vị</strong></h2>
					<p>Danh sách các đơn vị hành chính</p>            
					<table class="table table-striped" style="font-size: 16px; text-align: center;">
						...
						<tr>
							...
							<th style="text-align: center;"><h4><strong>Cấp</strong></h4></th>
							<th style="text-align: center;"><h4><strong>Tên đơn vị</strong></h4></th>
							<th style="text-align: center;"><h4><strong>Địa chỉ</strong></h4></th>
                            <th style="text-align: center;"><h4><strong>Email</strong></h4></th>
                            <th style="text-align: center;"><h4><strong>SĐT</strong></h4></th>
						...
						<tr id="row-<?php echo $local['id']; ?>">
							...
							<td><?php echo $local['id_parent'] ?></td>
							<td><?php echo $local['tendv'] ?></td>
                            <td><?php echo $local['diachi'] ?></td>
                            <td><?php echo $local['email'] ?></td>
                            <td><?php echo $local['sdt'] ?></td>
							<td><a href="<?php echo base_url('Donvi_Controller/sua_donvi/'.$local['id']);?>"><input type="submit" class="btn btn-primary" name="sua" value="Sửa" style="height: 32px;" /></a>
							<a class="deldonvi" data-id="<?php echo $local['id']; ?>" href="javascript:void(0)"><input type="submit" class="btn btn-primary" name="xoa" value="Xóa" style="height: 32px;"/></a></td>
						</tr>
						<?php
							}
						?>
				...
				<div class="row-2" style="width: 145px; float: right; ">
					<p><a href="<?php echo base_url('Donvi_Controller/them_donvi') ?>"><input type="submit" class="btn btn-primary" name="them" value="Thêm mới đơn vị" /></p>		
				</div>
				<div class="row">
        ...
        $('.deldonvi').on('click', function() {
          var _id = $(this).data('id');
            ...
            if(confirm('Bạn muốn xóa đơn vị này ?')){
                $.post(
                '<?php echo base_url('Donvi_Controller/xoa_donvi/');?>',
                {
                    id: _id
                }, function(response) {
                    if(response){
